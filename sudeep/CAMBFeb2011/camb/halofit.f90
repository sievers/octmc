...
contains
  ...
  subroutine NonLinear_GetNonLinRatios(CAMB_Pk)
    ...
    !!BR09 putting neutrinos into the matter as well, not sure if this is correct,
    !!but at least one will get a consisent omk.
    ...
    do itf = 1, CAMB_Pk%num_z
       ...
       !RdP
       om_m = omega_m(a, omm0, CP%omegav, w0_de, wa_de)  
       om_v = omega_v(a, omm0, CP%omegav, w0_de, wa_de)
...
101    continue
  ...
  function omega_m(aa,om_m0,om_v0,w0val,waval)
    implicit none
    real(dl) omega_m,omega_t,om_m0,om_v0,aa,w0val,waval
    
    omega_t=1.0+(om_m0+om_v0-1.0)/(1-om_m0-om_v0+om_v0*((aa)**(-1.0-3.0*w0val - 3.0*waval))&
         & * exp(-3. * waval * (1. - aa)) +om_m0/aa)
    !omega_t=1.0+(om_m0+om_v0-1.0)/(1-om_m0-om_v0+om_v0*((aa)**(-1.0-3.0*wval))+om_m0/aa)
    omega_m=omega_t*om_m0/(om_m0+om_v0*((aa)**(-3.0*w0val - 3.0 * waval)) * exp(-3. * waval * (1. - aa)))
  ...
  function omega_v(aa,om_m0,om_v0,w0val,waval)      
    implicit none
    real(dl) aa,omega_v,om_m0,om_v0,omega_t,w0val, waval
    real(dl) temp

    omega_t=1.0+(om_m0+om_v0-1.0)/(1-om_m0-om_v0+om_v0*((aa)**(-1.0-3.0*w0val - 3.0*waval))&
         & * exp(-3. * waval * (1. - aa)) +om_m0/aa)
    !omega_t=1.0+(om_m0+om_v0-1.0)/(1-om_m0-om_v0+om_v0*((aa)**(-1.0-3.0*wval))+om_m0/aa)
    temp = omega_t*om_v0 * ((aa)**(-3.0-3.0*w0val - 3.0 * waval)) * exp(-3. * waval * (1. - aa))
    omega_v = om_v0 * ((aa)**(-3.0-3.0*w0val - 3.0 * waval)) * exp(-3. * waval * (1. - aa)) + om_m0/aa/aa/aa
    omega_v = temp/omega_v
