       ...
       contains
         ...
         function grho_de(a) !8 pi G a^4 rho_de
            ...
            real(dl), intent(IN) :: a
               ...
               grho_de = grhov * a**(1._dl - 3. * w0_de - 3. * wa_de) * exp(-3. * wa_de * (1._dl - a))
...
!  Compute expansion rate from: grho 8*pi*rho*a**2
		...
		!RdP
        gpres=(grhog_t+grhor_t)/3+grhov_t * wde
...
! have to get z from eta for numerical stability
        ...
        if (w0_de /= -1 .and. w_Perturb) then
           ...
           ayprime(EV%w_ix)= -3*adotoa*(cs2_lam-wde)*(clxq+3*adotoa*(1.+wde)*vq/k) &
               -(1.+wde)*k*vq -(1.+wde)*k*z

           ayprime(EV%w_ix+1) = -adotoa*(1-3*cs2_lam)*vq + k*cs2_lam*clxq/(1+wde)
...
!  Use explicit equation for vb if appropriate
    ...
    !  zeroth order approximation to vbdot + the pig term
           ...
           !  ddota/a
		     !RdP
             gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*wde
...
! Also calculate gpres: 8*pi*p*a**2
        ...
        grho=grhob_t+grhoc_t+grhor_t+grhog_t+grhov_t
		!RdP
        gpres=(grhog_t+grhor_t)/3._dl+grhov_t*wde
